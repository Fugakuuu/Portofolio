...
export const LogoLoop = memo(
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        return (
          <li
            className={cx(
              "flex-none text-[length:var(--logoloop-logoHeight)] leading-[1] text-foreground transition-colors duration-300",
              isVertical
                ? "mb-[var(--logoloop-gap)]"
                : "mr-[var(--logoloop-gap)]",
              scaleOnHover && "overflow-visible group/item"
    ...
    const maskStyle = useMemo<CSSProperties>(() => {
      if (!fadeOut || fadeOutColor) return {};
      const fadeSize = "clamp(20px, 8%, 80px)";
      if (isVertical) {
        return {
          maskImage: `linear-gradient(to bottom, transparent 0%, black ${fadeSize}, black calc(100% - ${fadeSize}), transparent 100%)`,
          WebkitMaskImage: `linear-gradient(to bottom, transparent 0%, black ${fadeSize}, black calc(100% - ${fadeSize}), transparent 100%)`,
        };
      }
      return {
    ...
    const containerStyle = useMemo<CSSProperties>(
      ...
      [width, cssVariables, maskStyle, style, isVertical]
    );
